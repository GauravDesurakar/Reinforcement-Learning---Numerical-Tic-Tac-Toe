...
B. Define the action space for each state. (Be careful in defining actions. The actions are not the same for each state)

C. Define the winning states: the sum of three numbers in a row, column or diagonal is 15.

D. Define the terminal states (win,tie,loss)

E. Build the reward structure as below:
